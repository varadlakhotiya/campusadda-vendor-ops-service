...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    await VendorScope.initVendorSelect("vendorSelect", async () => {
      ...
      await loadMlOperationalInsights();
      startAutoRefresh();
    });
  } catch (err) {
...
function bindOrderEvents() {
  document.getElementById("refreshOrdersBtn")?.addEventListener("click", async () => {
    ...
    await loadMlOperationalInsights(false);
    if (selectedOrderId) {
  ...
  orders.forEach((order) => {
    ...
    card.innerHTML = `
      <div class="order-card-head">
        <strong>${Utils.escapeHtml(order.orderNumber ?? "-")}</strong>
      ...
      <div class="order-card-grid">
        <div class="order-kv">
          <span>Customer</span>
          <strong>${Utils.escapeHtml(order.customerName ?? "-")}</strong>
        </div>
        <div class="order-kv">
          <span>Phone</span>
          <strong>${Utils.escapeHtml(order.customerPhone ?? "-")}</strong>
        </div>
        <div class="order-kv">
          <span>Total</span>
        ...
        <div class="order-kv">
          <span>Source</span>
          <strong>${Utils.escapeHtml(order.orderSource ?? "-")}</strong>
        </div>
      </div>
      ...
      ${
        order.notes
          ? `<div class="order-notes"><span>Notes</span><p>${Utils.escapeHtml(order.notes)}</p></div>`
          : ""
      }
    ...
    summary.innerHTML = `
      <div class="detail-summary-grid">
        ...
        <div class="order-kv">
          ...
          <strong>${Utils.escapeHtml(order.paymentStatus ?? "-")}</strong>
        </div>
      </div>
      <div class="order-notes detail-notes">
        <span>Customer Notes</span>
        <p>${Utils.escapeHtml(order.notes ?? "No notes added by customer.")}</p>
      </div>
    `;
  }
      ...
      itemsBox.innerHTML = items
        .map((item) => `
          <div class="detail-row">
            <div>
              <strong>${Utils.escapeHtml(item.itemNameSnapshot ?? item.itemName ?? "-")}</strong>
              <div class="muted">
                Qty ${Utils.escapeHtml(item.quantity ?? "-")} • ${Utils.formatCurrency(item.unitPrice ?? 0)} each
              </div>
              ${
                item.specialInstructions
                  ? `<div class="detail-note">Instructions: ${Utils.escapeHtml(item.specialInstructions)}</div>`
                  : ""
              }
            </div>
        ...
        .map((entry) => `
          <div class="history-row">
            ...
            <div class="history-content">
              <strong>${Utils.escapeHtml(entry.toStatus ?? "-")}</strong>
              <div class="muted">${Utils.escapeHtml(entry.remarks ?? "Status updated")}</div>
              <div class="muted">
...
function clearMlInsightBox() {
  const box = document.getElementById("orderMlInsightBox");
  if (box) {
    box.innerHTML = `Select a vendor to view demand forecast, anomaly count, reorder pressure, and peak-hour guidance.`;
    box.className = "insight-stack empty-state";
  }
}

async function loadMlOperationalInsights(showToastOnError = true) {
  const vendorId = await getSelectedVendorId();
  if (!vendorId) {
    latestMlInsights = null;
    clearMlInsightBox();
    Utils.setText("ordersAnalyticsTotal", 0);
    Utils.setText("ordersAnalyticsCompleted", 0);
    Utils.setText("ordersPeakHour", "—");
    Utils.setText("ordersRiskSignals", 0);
    return;
  }
  ...
  try {
    const [
      ...
      Api.get(`/vendors/${vendorId}/analytics/hourly-sales?fromDate=${fromDate}&toDate=${toDate}`),
    ...
    latestMlInsights = {
      ...
      reorders: reorderResponse.data || [],
      anomalies: anomalyResponse.data || []
    };

    renderMlInsightSummary();
    renderMlInsightBox();
  } catch (err) {
    if (showToastOnError) {
      Utils.showMessage(err.message || "Failed to load ML operational insights", "error");
    }
  }
}

function renderMlInsightSummary() {
  const analytics = latestMlInsights?.analytics || {};
  const hourly = latestMlInsights?.hourly || [];
  const reorders = latestMlInsights?.reorders || [];
  const anomalies = latestMlInsights?.anomalies || [];

  const peak = hourly.reduce((best, row) => {
    const currentRevenue = Number(row.revenue || 0);
    if (!best || currentRevenue > best.revenue) {
      return {
        hour: Number(row.salesHour),
  ...
  Utils.setText("ordersAnalyticsTotal", analytics.totalOrders ?? 0);
  Utils.setText("ordersAnalyticsCompleted", analytics.completedOrders ?? 0);
  Utils.setText("ordersPeakHour", peak ? Utils.formatHourLabel(peak.hour) : "—");

  const riskSignals =
    reorders.filter((item) => Number(item.suggestedReorderQty || 0) > 0).length +
    anomalies.filter((item) => String(item.status || "").toUpperCase() !== "RESOLVED").length;
  Utils.setText("ordersRiskSignals", riskSignals);
}

function renderMlInsightBox() {
  const box = document.getElementById("orderMlInsightBox");
  if (!box) return;

  const analytics = latestMlInsights?.analytics || {};
  const hourly = latestMlInsights?.hourly || [];
  const topItems = latestMlInsights?.topItems || [];
  const reorders = latestMlInsights?.reorders || [];
  const anomalies = latestMlInsights?.anomalies || [];

  const openReorders = reorders.filter((item) => Number(item.suggestedReorderQty || 0) > 0);
  const openAnomalies = anomalies.filter((item) => String(item.status || "").toUpperCase() !== "RESOLVED");

  if (!topItems.length && !openReorders.length && !openAnomalies.length) {
    box.className = "insight-stack empty-state";
    box.innerHTML = "No ML operational insight available yet for this vendor.";
    return;
  }
  ...
  const peakHours = hourly
    ...
    .slice(0, 3)
    .map((row) => Utils.formatHourLabel(Number(row.salesHour)));

  box.className = "insight-stack";
  box.innerHTML = `
    <div class="insight-card">
      <div class="insight-title">Demand Snapshot</div>
      <div class="insight-value">${Utils.formatNumber(analytics.totalOrders ?? 0)} orders</div>
      <div class="insight-subtext">Last 14 days • ${Utils.formatCurrency(analytics.grossRevenue ?? 0)} gross revenue</div>
    </div>

    <div class="insight-card">
      <div class="insight-title">Peak Windows</div>
      <div class="insight-value">${peakHours.length ? peakHours.join(", ") : "No peak hour data"}</div>
      <div class="insight-subtext">Use these windows for preparation planning and staff readiness.</div>
    </div>
      ...
      <div class="insight-list">
        ${(topItems.slice(0, 3).map((item) => `
          <div class="insight-list-row">
            <span>${Utils.escapeHtml(item.itemName ?? "-")}</span>
            <strong>${Utils.formatNumber(item.quantitySold ?? 0)}</strong>
          </div>
        `).join("")) || `<div class="muted">No top-item data available.</div>`}
      </div>
    </div>
    ...
    <div class="insight-card">
      ...
      <div class="insight-subtext">
        ${openReorders.length} reorder recommendation(s) and ${openAnomalies.length} anomaly signal(s) are currently open.
      </div>
      <div class="insight-list">
        ${(openReorders.slice(0, 2).map((item) => `
          <div class="insight-list-row">
            <span>Reorder item #${item.inventoryItemId}</span>
            <strong>${Utils.formatNumber(item.suggestedReorderQty ?? 0)}</strong>
          </div>
        `).join("")) || ""}
        ${(openAnomalies.slice(0, 2).map((item) => `
          <div class="insight-list-row">
            <span>${Utils.escapeHtml(item.menuItemName ?? `Item #${item.menuItemId}`)}</span>
            <strong>${Utils.escapeHtml(item.severity ?? "Open")}</strong>
          </div>
        `).join("")) || ""}
      </div>
    </div>
