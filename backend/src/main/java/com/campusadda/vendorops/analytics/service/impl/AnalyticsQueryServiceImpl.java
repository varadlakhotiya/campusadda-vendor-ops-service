...
public class AnalyticsQueryServiceImpl implements AnalyticsQueryService {
    ...
    public RevenueSummaryResponse getRevenueSummary(Long vendorId, LocalDate fromDate, LocalDate toDate) {
        ...
        BigDecimal net = rows.stream()
                .map(r -> r.getNetRevenue() == null ? BigDecimal.ZERO : r.getNetRevenue())
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        int totalOrders = rows.stream()
                .mapToInt(r -> r.getTotalOrders() == null ? 0 : r.getTotalOrders())
                .sum();
    ...
    public OrderStatusSummaryResponse getOrderStatusSummary(Long vendorId, LocalDate fromDate, LocalDate toDate) {
        ...
        int completed = rows.stream()
                .mapToInt(r -> r.getCompletedOrders() == null ? 0 : r.getCompletedOrders())
                .sum();

        int cancelled = rows.stream()
                .mapToInt(r -> r.getCancelledOrders() == null ? 0 : r.getCancelledOrders())
                .sum();
